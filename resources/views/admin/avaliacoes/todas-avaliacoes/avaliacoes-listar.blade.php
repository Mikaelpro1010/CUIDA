...
@section('titulo', 'Todas as Avaliações')

@section('content')
    <div class="d-flex justify-content-between">
        <h1 class="text-primary fs-3"> Todas as Avaliações({{ $avaliacoes->total() }})</h1>
    </div>
    <hr>
    <form class="" action="{{ route('get-avaliacoes-list') }}" method="GET">
        <div class="d-flex flex-wrap gap-2">
            ...
            <div class="flex-fill">
                <label class="fw-bold" for="pesquisa">IP</label>
                <input id="pesq_ip" class="form-control" type="text" name="pesq_ip" placeholder="ex. 192.168.1.1"
                    value="{{ request()->pesq_ip }}" pattern="^(\d{1,3}\.){3}\d{1,3}$">
            </div>
            <div class="flex-fill">
            ...
            <tbody class="table-group-divider">
                @forelse ($avaliacoes as $avaliacao)
                    <tr>
                        ...
                        <td>
                            {{ $avaliacao->avaliador_ip }}
                        </td>
                        <td>
                            {{ $avaliacao->user_agent }}
                        </td>
                    </tr>
