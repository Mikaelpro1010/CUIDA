...
@section('content')
    <div>
        ...
        @if (auth()->user()->can(permissionConstant()::GERENCIAR_TIPOS_AVALIACAO_LIST))
            <h3>Avaliações:</h3>
            <div class="row gap-2">
                @can(permissionConstant()::GERENCIAR_TIPOS_AVALIACAO_LIST)
                    <div class="col-md-3">
                        <a href="{{ route('get-tipo-avaliacao-list') }}" class="btn btn-success w-100">
                            <i class="fa-5x fa-solid fa-list-check"></i><br>
                            <b>Tipos de Avaliação</b>
                        </a>
                    </div>
                @endcan
            </div>
