...
//     {
//         if (auth()->user()->cant(Permission::UNIDADE_SECRETARIA_ACCESS_ANY_SECRETARIA)) { // Se o usuário não tem permissão para acessar qualquer secretaria
//             $abort = true;
//             foreach (auth()->user()->secretarias as $secretaria) {  // verifica se o usuario tem permissão para acessar a secretaris da unidade
//                 if ($secretaria->is($this->secretaria)) {  // se o usuario tem 
//                     $abort = false;
//                     break;
//                 }
//             }
...
class Unidade extends Model
{
    ...
    public function userCanAccess()
    {
        if (auth()->user()->cant(Permission::UNIDADE_SECRETARIA_ACCESS_ANY_SECRETARIA)) { // Se o usuário não tem permissão para acessar qualquer secretaria
            $abort = true;
            foreach (auth()->user()->secretarias as $secretaria) {  // verifica se o usuario tem permissão para acessar a secretaria da unidade
                if ($secretaria->is($this->secretaria)) {  // se o usuario tem 
                    $abort = false;
                    break;
                }
    ...
    public function getResumoSetorById($setorId): Collection
    {
        return $this->setores()->find($setorId)->getResumo();
    }
